<template>
  <el-form :size="size">
    <el-form-item>
      <el-radio v-model="radioValue" :label="1"> {{$t('cron.everyMinute')}}，{{$t('cron.allowedWildcards')}} [, - * /] </el-radio>
    </el-form-item>

    <el-form-item>
      <el-radio v-model="radioValue" :label="2">
        {{ $t('cron.cycleFrom') }}
        <el-input-number class="mx-1em my-0" v-model="cycle01" :min="0" :max="58" controls-position="right" @focus="radioChange(2)" />
        {{ $t('cron.to') }}
        <el-input-number
          class="mx-1em my-0"
          v-model="cycle02"
    ...
    <el-form-item>
      <el-radio v-model="radioValue" :label="3">
        {{ $t('cron.cycleFrom') }}
        <el-input-number class="mx-1em my-0" v-model="average01" :min="0" :max="58" controls-position="right" @focus="radioChange(3)" />
        {{ $t('cron.minute') }}{{ $t('cron.start') }}，{{ $t('cron.every') }}
        <el-input-number
          class="mx-1em my-0"
          v-model="average02"
          :min="1"
          ...
          @focus="radioChange(3)"
        />
        {{ $t('cron.minute') }}{{ $t('cron.executeOnce') }}
      </el-radio>
    </el-form-item>

    <el-form-item class="start">
      <el-radio v-model="radioValue" :label="4" style="margin-right: 20px">{{ $t('cron.designate') }} {{ $t('cron.minute') }} </el-radio>
      <div class="flex">
        <el-checkbox-group v-model="checkboxList" class="grid grid-cols-10 justify-items-stretch">
          <el-checkbox v-for="item in 60" :key="item" :value="item - 1" :label="zeroFill(item - 1)" />
        </el-checkbox-group>
      </div>
    </el-form-item>
  </el-form>
...
<script setup name="CronMin" lang="ts">
